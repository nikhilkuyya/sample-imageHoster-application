...
public class UserController {
    ...
    public String registration(
            @RequestParam(name = "passwordInvalid", defaultValue = "false", required = false) Boolean isPasswordValid,
            @RequestParam(name = "fieldMissing", defaultValue = "false", required = false) Boolean isFieldsMissing,
            Model model) {
        User user = new User();
        UserProfile profile = new UserProfile();
        user.setProfile(profile);
        model.addAttribute("User", user);
        model.addAttribute("passwordTypeError", isPasswordValid);
        model.addAttribute("fieldMissingError", isFieldsMissing);
    ...
    public String registerUser(User user, Model model) {
        if (userService.isValidUser(user)) {
            if (userService.isValidPassword(user.getPassword())) {
                ...
                return "redirect:/users/registration?" + "passwordInvalid=" + true;
            }
        } else {
            return "redirect:/users/registration?" + "fieldMissing=" + true;
        }
